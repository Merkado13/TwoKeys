...
namespace TMPro
{
    ...
    public class TMP_Animated : TextMeshProUGUI
    {
        ...
        {
            text = string.Empty;
            vertexJitter.initChar = -1;
            vertexJitter.endChar = -1;
            ...
            {
                return tag.StartsWith("speed=") || tag.StartsWith("pause=") || tag.StartsWith("/jitter") || tag.StartsWith("jitter=");
            }
            ...
            IEnumerator Read()
            {
                ...
                WaitForSeconds EvaluateTag(string tag)
                {
                    if (tag.Length > 0)
                    {
                        ...
                        }else if (tag.StartsWith("jitter"))
                        {
                            string[] values = tag.Split('=');
                            string[] actualValues = values[1].Split(',');
                            vertexJitter.AngleMultiplier = float.Parse(actualValues[0]);
                            vertexJitter.SpeedMultiplier = float.Parse(actualValues[1]);
                            vertexJitter.CurveScale = float.Parse(actualValues[2]);
                            vertexJitter.initChar = maxVisibleCharacters - 1;
                            vertexJitter.endChar = displayText.Length;
                        }else if (tag.StartsWith("/jitter"))
                        {
                            vertexJitter.endChar = maxVisibleCharacters - 1;
                        }
                    }
                    return null;
                }
